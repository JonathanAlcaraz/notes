...
\begin{lma}
  If \(\{X_i\}\) is a pairwise disjoint family of sets, then the inclusions \( \{ \iota_i : F(X_i) \to F(\cup X_i) \} \) are a coproduct in 
\end{lma}

\begin{prf}
  \begin{minipage}[t][7em][c]{.35\textwidth}
    \begin{tikzcd}
      X_i \ar[r, "r_i"] \ar[d, "s_i"]  & \bigcup X_i \ar[d, "s"] \ar[ddl, dashed, "g'"' near start] \\
      F(X_i) \ar[r, crossing over, "\iota_i"] \ar[d, "g_i"] & F(\bigcup X_i) \ar[dl, bend left=5, "g"] & \\
      H &
    \end{tikzcd}
  \end{minipage}
  The maps \(g_i\circ s_i\) induce a map 
\end{prf}
